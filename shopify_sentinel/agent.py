...
# Import sub-agents
from .agents.friction_scout import frictionscout_playwright_mcp
from .agents.luxury_friction_scout import luxury_friction_scout_playwright
from .agents.navigation_graph import navigation_graph__deadend_detector_playwright__shopify_admin_2
from .agents.performance_auditor import performance__visual_stability_auditor_lighthouse
from .agents.search_visibility import search_visibility__index_hygiene_google_search_console_2
from .agents.behavioral_funnel_analyst import behavioral_funnel_analyst_google_analytics
from .agents.release_regression_watcher import release_regression_watcher_ga__gsc__lighthouse
from .agents.luxury_copy_editor import luxury_copy__microtrust_editor_shopify_admin__playwright
from .agents.catalog_integrity import catalog_integrity__merchandising_control_shopify_admin

shopify_sentinel__coordinator__google_search_agent = LlmAgent(
  name='ShopifySentinel__Coordinator__google_search_agent',
...
shopify_sentinel__coordinator__url_context_agent = LlmAgent(
  name='ShopifySentinel__Coordinator__url_context_agent',
  model='gemini-2.5-pro',
  description=(
...
root_agent = LlmAgent(
  name='ShopifySentinel__Coordinator',
  model='gemini-2.5-pro',
  description=(
      """Shopify Sentinel is a coordinator agent that improves conversion and brand credibility by orchestrating specialized subagents connected via MCP tools (Playwright, Lighthouse, Google Analytics, Google Search Console, Shopify Admin). It continuously produces ranked, implementation-ready tickets that remove friction, fix structural errors, and sharpen presentation quality without diluting exclusivity. It treats UI quality as product quality, and optimizes for decisive, status-aware buyers who abandon anything that feels templated, cluttered, or uncertain.
...
Prioritization heuristic (strict)
  ...
  tools=[
    agent_tool.AgentTool(agent=shopify_sentinel__coordinator__google_search_agent),
    agent_tool.AgentTool(agent=shopify_sentinel__coordinator__url_context_agent),
